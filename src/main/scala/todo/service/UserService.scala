...
trait UserServiceLive extends UserService with AuthorizationService {

  override val userService: UserService.Service = new UserService.Service {
    ...
    override def handleUserAuthentication(
      ...
      checkCredentials(credentials.name, determineHash(credentials.password))
        .flatMap {
          case Some(_) =>
            ZIO.succeed(Right(AuthenticationResponse(authorizationService.generateToken(credentials.name))))
